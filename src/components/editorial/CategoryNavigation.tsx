...
import { StaggerReveal, StaggerItem, ScrollReveal } from "@/components/ui/ScrollReveal";
...
export const CategoryNavigation: React.FC = () => {
  return (
    <section className="w-full py-16 sm:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <ScrollReveal className="flex flex-col sm:flex-row sm:items-end justify-between pb-8 mb-8 border-b border-white/10">
        <div>
      ...
      <StaggerReveal className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {CATEGORIES.map((cat, i) => (
          <StaggerItem key={cat.name}>
            <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.25, ease: [0.25, 0.46, 0.45, 0.94] }}>
              <Link
                href={cat.href}
                className="group relative aspect-[3/4] overflow-hidden bg-zinc-900 border border-white/10 hover:border-white/40 transition-colors duration-300 flex flex-col justify-end p-6 select-none block"
              >
                <Image
                  src={cat.image}
                  alt={cat.name}
